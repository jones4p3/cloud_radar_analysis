import pandas as pd
import numpy as np
def uptime_from_interval_times(interval_times, interval_start_time, interval_end_time, max_sampling_time=10, debug=False):
    """
    ...
    for time_idx_in_interval in range(interval_times.size):
        ...
        if time_idx_in_interval == 0:
            ...
            if interval_times[time_idx_in_interval] > interval_start_time:
                if debug:
                    print(f"First sample {interval_times[time_idx_in_interval]} after interval start {interval_start_time}")
                ...
                passed_seconds_after_starting_time = (interval_times[time_idx_in_interval] - interval_start_time)
                ...
                if debug:
                    print(f"  Total covered_seconds (s): {covered_seconds} from {total_interval_in_seconds} [{(covered_seconds/total_interval_in_seconds):.4f}]" )
        ...
        # Last sample in the interval
            ...
            if seconds_until_interval_end < max_sampling_time: # If it is less than max_sampling_time, we can only add the remaining time until the interval end assuming the radar was measuring in this interval and saving in the next at 00:00:00 or 00:00:01 etc..
        ...
        # All other samples in the interval
            ...
            current_sample_end_time = interval_times[time_idx_in_interval]
            difference_between_samples = (current_sample_end_time - previous_sample_end_time)
            if current_sample_end_time < interval_start_time:
                print(f"⚠️ WARNING: Sample {time_idx_in_interval}: {current_sample_end_time} before interval start {interval_start_time}")
                current_sample_end_time = interval_start_time
            if current_sample_end_time > interval_end_time:
                print(f"⚠️ WARNING: Sample {time_idx_in_interval}: {current_sample_end_time} after interval end {interval_end_time}")
                continue

            # Check for downtime longer than max_sampling_time
            if difference_between_samples > (max_sampling_time):
                if debug:
                    print(f"  Downtime longer than max_sampling_time between sample {time_idx_in_interval-1} and {time_idx_in_interval}: {difference_between_samples} seconds")
                covered_seconds += max_sampling_time
                start_cut_off_count += 1
            else:
    ...
    if debug:
        print(f"--------- Uptime in interval: {uptime_in_interval:.2f} (covered_seconds {covered_seconds} seconds of {total_interval_in_seconds} seconds)")
    return float(uptime_in_interval),start_cut_off_count, end_assumed_added_count, end_assumed_added_seconds

def calculate_uptime(ds, max_sampling_time_in_seconds, sampling_interval_in_minutes, threshold_for_interval, cfg=None): 
    ...
    # Build the interval start times
    campaign_start_time, campaign_end_time = ds.attrs["campaign_start_time"], ds.attrs["campaign_end_time"]
    interval_start_times = pd.date_range(start=campaign_start_time, end=campaign_end_time, freq=sampling_interval_in_minutes_str)
    
    # Sort dataset time values and create pandas datetime index and series
    dataset_time_values = pd.to_datetime(ds["time"].sortby("time").values) # Create sorted pandas datetime index
    dataset_time_series = pd.Series(dataset_time_values, index=dataset_time_values)

    # Group observed times into intervals based on sampling interval
    grouped_dataset_time_series = dataset_time_series.groupby(pd.Grouper(freq=sampling_interval_in_minutes_str, origin=campaign_start_time)) # Groups the times into intervals, where the origin is the campaign start time e.g every time between 2025-02-01 00:00:00 and 2025-02-01 00:09:59 will be grouped together if the sampling interval is 10 minutes
    # Get uptime for each interval
    uptime_per_interval = []
    total_start_cut_off_count = 0
    total_end_added_count = 0
    # Iterrating thorugh the interval start times 18:10, 18:20, 18:30 etc...
    for interval_start_time in interval_start_times:
        if debug: print(f"Processing interval starting at {interval_start_time}")
        interval_end_time = interval_start_time + pd.Timedelta(sampling_interval_in_minutes_str)  # End time of the interval

        # Get times within the current interval
        if debug: print(f"Getting times for interval starting at {interval_start_time}")
        try:
            interval_times = grouped_dataset_time_series.get_group(interval_start_time).values
            if debug: print(f"Found {len(grouped_dataset_time_series.get_group(interval_start_time).values)} samples in interval starting at {interval_start_time}")
        except KeyError: # KeyError occurs if the dataset has no samples in this interval 
            if debug: print(f"No samples found in interval starting at {interval_start_time}")
            interval_times = np.array([])

        # Check interval times
        if interval_times.size == 0:
            if debug: print(f"No interval times found for interval starting at {interval_start_time}")
            ...
            if debug: print(f"Interval times for interval starting at {interval_start_time}: {interval_times[0:2]} ... {interval_times[-2:]}")
            # Calculate uptime for the n-th interval
            uptime_in_interval, start_cut_off_count, end_added_count, end_added_seconds = uptime_from_interval_times(
                interval_times,
        ...
        uptime_per_interval.append((interval_start_time, interval_end_time, uptime_in_interval, ignore_time, len(interval_times), start_cut_off_count, end_added_count, end_added_seconds))

    return pd.DataFrame(
        uptime_per_interval,
        columns=["interval_start_time", "interval_end_time", "uptime", "ignore_time", "n_samples", "start_cut_off_count", "end_added_count", "end_added_seconds"],
    )
