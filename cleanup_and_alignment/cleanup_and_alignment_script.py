...
sampling_interval_in_minutes = 10
max_sampling_time_in_seconds = 30
threshold_for_interval = 0.90 # 90% of uptime in the interval is the threshold
...
def cleanup_and_align_datasets(radar_datasets, start_time, end_time):
    # Define time range & calculating total minutes in range
    campaign_start_time = np.datetime64(start_time)
    campaign_end_time = np.datetime64(end_time)
    time_range_total_minutes = ((np.datetime64(campaign_end_time)) - (np.datetime64(campaign_start_time)) + np.timedelta64(1,'s')) / np.timedelta64(1,'m')
    print(f"Using full campaign time range from {campaign_start_time} to {campaign_end_time}")
    ...
    for idx, (radar_name, radar_data) in enumerate(radar_datasets.items()):
        ...
        ds_total_minutes = ((np.datetime64(ds_end) - np.datetime64(ds_start)) + np.timedelta64(1,'s')) / np.timedelta64(1, 'm')
        ...
        dataset_uptime_df = calculate_uptime(ds, max_sampling_time_in_seconds=max_sampling_time_in_seconds, sampling_interval_in_minutes=sampling_interval_in_minutes, threshold_for_interval=threshold_for_interval)
    ...
    unique_total_downtime_df = total_downtime_df.drop_duplicates(subset=["interval_start_time"]).reset_index(drop=True).sort_values(by="interval_start_time") # Remove duplicate intervals across radars, keeps first occurrence
    unique_total_uptime_df = total_uptime_df.drop_duplicates(subset=["interval_start_time"]).reset_index(drop=True).sort_values(by="interval_start_time") # Remove duplicate intervals across radars, keeps first occurrence
    # "Substracting" downtime intervals from uptime intervals to get cleaned uptime intervals
    cleaned_uptime_df = unique_total_uptime_df[~unique_total_uptime_df["interval_start_time"].isin(unique_total_downtime_df["interval_start_time"])]
    cleaned_uptime_start_cut_off_minutes = cleaned_uptime_df["start_cut_off_count"].sum() / 2 # Each count is 30 seconds, so divide by 2 to get minutes
    cleaned_uptime_end_added_minutes = cleaned_uptime_df["end_added_seconds"].sum().astype(float) / 60 # Each row is in seconds, so divide by 60 to get minutes
    cleaned_uptime_total_minutes = (len(cleaned_uptime_df) * sampling_interval_in_minutes)
    print(f"Total downtime size: {len(total_downtime_df)}, Total uptime size: {len(total_uptime_df)}, Total combined: {len(total_downtime_df) + len(total_uptime_df)}")
    print(f"Unique downtime size: {len(unique_total_downtime_df)}, Unique uptime size: {len(unique_total_uptime_df)}, Unique combined: {len(unique_total_downtime_df) + len(unique_total_uptime_df)}")
    print(f"Cleaned uptime size: {len(cleaned_uptime_df)}")
    print(f"Cleaned uptime intervals cover from {cleaned_uptime_df['interval_start_time'].min()} to {cleaned_uptime_df['interval_end_time'].max()}")
    print(f"Total uptime: {len(cleaned_uptime_df) * sampling_interval_in_minutes} minutes -- {(len(cleaned_uptime_df) / 4032)*100:.2f}% of total campaign time")
    print(f"Cleaned uptime starting cut-offs: {cleaned_uptime_start_cut_off_minutes} minutes -- {(cleaned_uptime_start_cut_off_minutes / cleaned_uptime_total_minutes)*100:.2f}% of cleaned uptime")
    print(f"Cleaned uptime ending added time: {cleaned_uptime_end_added_minutes} minutes -- {(cleaned_uptime_end_added_minutes / cleaned_uptime_total_minutes)*100:.2f}% of cleaned uptime")

    # Create a combined dataframe of unique downtime and uptime
    unique_total_df = pd.concat([unique_total_downtime_df, unique_total_uptime_df], ignore_index=True)
    unique_total_df["ignore_time"] = unique_total_df["ignore_time"].astype(bool) # Making double sure the column is boolean
    unique_total_df = unique_total_df.sort_values(["interval_start_time", "ignore_time"], ascending=[True, False]) # Sorting by time and putting ignore_time=True first
    # Now drop_duplicates to keep only the first occurrence (which is ignore_time=True if there is a duplicate)
    unique_total_df = unique_total_df.drop_duplicates(subset=["interval_start_time"], keep="first").reset_index(drop=True).sort_values(by="interval_start_time")

    uptime_results["Total"] = unique_total_df # Store combined results under "Total" key
    ...
    dataset = [name for name in radar_datasets.keys()] + ["Total"]
    label_names = []


    for row_idx, radar_name in enumerate(dataset):
        df = uptime_results[radar_name]
        band = radar_datasets[radar_name].attrs["band"] if radar_name in radar_datasets else "Common Periods"
    ...
    plt.savefig(f"uptime_downtime_{sampling_interval_in_minutes}min_{max_sampling_time_in_seconds}s_threshold_{int(threshold_for_interval*100)}.png", dpi=300, bbox_inches='tight')

    # -------------------------------------------------------
    # HEIGHT RESOLUTION 
    # -------------------------------------------------------
    highest_minimum_height = {
        "radar": "",
        "height": -np.inf
    }
    lowest_maximum_height = {
        "radar": "",
        "height": np.inf
    }
    for radar, ds in radar_datasets.items():
    ...
    print(f"Radar with lowest min height: {highest_minimum_height['radar']} at {highest_minimum_height['height']} m")
    print(f"Radar with highest max height: {lowest_maximum_height['radar']} at {lowest_maximum_height['height']} m")
    ...
    print(f"Common height range for all radars: {common_min_height} m to {common_max_height} m")

    for radar, ds in radar_datasets.items():
        print(f"  ---------------- Slicing radar: {radar} ----------------")
        height_size = ds["height"].size
        height_1d = np.asarray(ds["height"].values).squeeze()
        idx = np.where((height_1d >= common_min_height) & (height_1d <= common_max_height))[0]
        ...
        radar_datasets[radar] = ds_sliced
    ...
    for idx, (radar, ds) in enumerate(radar_datasets.items()):
        # Grab ds information
        ...
        edges[-1] = height[-1] + 0.5 * (height[-1] - height[-2]) # Here n is the last index, and n-1 is the second last index

        # Calculate the differences between edges to get range gate sizes
        range_gate_sizes = np.diff(edges)
        rounded_range_gate_sizes = np.round(range_gate_sizes, decimals=rounding_tolerance)

        # Get unique range gate sizes, their first occurrence index and counts_per_unique_range_gate_size
        unique_range_gate_sizes, first_occurrence_index, counts_per_unique_range_gate_size = np.unique(rounded_range_gate_sizes, return_index=True, return_counts=True) # Get unique range gate sizes, their first occurrence first_occurrence_index and counts_per_unique_range_gate_size

        # Filter by the amount of counts
        correct_counts = np.where(counts_per_unique_range_gate_size > 1)
        assigned_range_sizes, assigned_range_sizes_index = (unique_range_gate_sizes[correct_counts], first_occurrence_index[correct_counts])

        print(f"------------ Radar: {radar_name} ------------")
        print(f"Range gate sizes [{len(unique_range_gate_sizes)}]:\t{unique_range_gate_sizes}")
        print(f"Counts: \t\t{counts_per_unique_range_gate_size:}")
        print(f"Index: \t\t\t{first_occurrence_index}\n")

        print(f"Correct Range gate sizes [{len(assigned_range_sizes)}]: {assigned_range_sizes}")
        ...
        for height_idx in range(height.size):
            # First chirp setting
            if assigned_range_sizes_index[0] <= height_idx < assigned_range_sizes_index[1]:
                assigned_range_sizes_with_height.append(assigned_range_sizes[0])
            # Second chirp setting
            elif assigned_range_sizes_index[1] <= height_idx < assigned_range_sizes_index[2]:
                assigned_range_sizes_with_height.append(assigned_range_sizes[1])
            # Third chirp setting
            elif assigned_range_sizes_index[2] <= height_idx < assigned_range_sizes_index[3]:
        ...
        print(f"Check for consistency: {height.size} vs {assigned_range_sizes_with_height.size}")
        ...
        radar_datasets[radar] = ds

        # Create plots
        ax[idx].plot(range_gate_sizes, height, ls="-", marker="o")
        ax[idx].plot(assigned_range_sizes_with_height, height, ls="--", color="red", label="Assigned range gate size")
        colors = plt.cm.tab10.colors
        for color_id, (range_size, range_index) in enumerate(zip(assigned_range_sizes, assigned_range_sizes_index)):
            if range_index > 0:
                range_index = range_index
            range_switch_height = height[range_index]
            ax[idx].axhline(y=range_switch_height, xmin=0, xmax=1,ls="--", color=colors[color_id], label=f"Range gate size switch [at {range_index}] to {range_size:.2f} m")
    ...
    plt.savefig(f"radar_range_gate_sizes_all_radars.pdf", dpi=450)
    ...
    def split_ds_by_intervals(
        ...
        for row in df.itertuples(index=False):
            ...
            in_interval = (times >= start) & (times < end) # Left-inclusive, right-exclusive [start, end)]
    ...
    for radar_slug, ds in radar_datasets.items():
        ...
        print(f"Original dataset length: {ds.sizes['time']} -- {ds_clean.sizes['time']} after cleaning -- {ds_ignored.sizes['time']} removed")
        print(f"Cleaned dataset length:  {ds_clean.sizes['time']}")
        n_removed_timesteps = ds.sizes["time"] - ds_clean.sizes["time"]
        print(f"Removed {n_removed_timesteps} timestamps. [{(n_removed_timesteps/ds.sizes['time'])*100:.2f}%]")
        ...
        print(f"--------------- OVER ALL DATASET INFORMATION FOR {radar_slug} ---------------")
        print(f"Time range: from {ds_clean.time.min().values} to {ds_clean.time.max().values} \t || Total time steps: {ds_clean.sizes['time']}")
        print(f"Height range: from {ds_clean.height.min().values} to {ds_clean.height.max().values} \t\t\t\t\t || Total height gates: {ds_clean.sizes['height']}")
        print(f"Range gate sizes included: {unique_range_gate_sizes} meters \t\t\t\t || Total unique sizes: {len(unique_range_gate_sizes)}")
        print(f"Sensitivity curve included  \t\t\t\t\t\t\t\t || Min to max: {ds_clean.sensitivity.min().values:.2f} to {ds_clean.sensitivity.max().values:.2f} dBZ")
        print("---------------------------------------------------------------")
    
        radar_datasets[radar_slug] = ds_clean
